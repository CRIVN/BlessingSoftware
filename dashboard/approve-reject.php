...
<?php 
   ...
   $con = connect();  
	...
	if (isset($_GET['breject_id'])) {
		...
		echo '<script>alert("Rechazado.")</script>';
	...
	if (isset($_GET['bapprove_id'])) {
		...
		if ($con->query($sql) === TRUE) {
			...
			$mail->Body = '<html><body>
	                Hola '.$cname.' . <br>
					Su reservacion ha sido confirmada por el restaurante. <br>
					Graciaas.
	                </body></html>'; 
	            $mail->addAddress($email, "Reservacion aprobada");
	            if($mail->send()) {
	            	echo  '<script>alert("Reservacion confirmada.")</script>';
	                echo '<script>window.location="booking-list.php"</script>';
	            }else{
