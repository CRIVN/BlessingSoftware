...
$con = connect();
	if (isset($_POST['regascus'])){
        ...
        if ($checkresult->num_rows > 0) {
        	echo '<script>alert("El restaurante con este correo electrónico ya existe.")</script>';
	        	...
	        	if (isset($_FILES['image'])) {
				 // files handle
					...
					$iquery="INSERT INTO `restaurant_info`(`restaurent_name`, `email`, `phone`, `address`, `logo`, `password`, `role`) 
			        		VALUES ('$fullname','$email','$phone','$address','$file_name','$password','$role');";
		        	if ($con->query($iquery) === TRUE) {
		        		echo '<script>alert("Nuevo restaurante anandido satisfactoriamente")</script>';
		        		echo '<script>window.location="register.php"</script>';
		        	}else {
		                echo "Error: " . $iquery . "<br>" . $con->error;
		            }
	...
	if (isset($_POST['regasres'])){
        ...
        $checkresult = $con->query($checkSQL);
        if ($checkresult->num_rows > 0) {
        	echo '<script>alert("Ya existe un restaurante con este correo")</script>';
        	echo '<script>window.location="register.php"</script>';
        }else{
				 ...
				 // files handle
				    ...
				    if ($extension =="jpg" || $extension =="png" || $extension =="jpeg"){
			        	...
			        	if ($con->query($iquery) === TRUE) {
			        			...
			        			echo '<script>alert("Restaurante anadido satisfactoriamente")</script>';
			        				echo '<script>window.location="login.php"</script>';
			    ...
			    //     				echo '<script>window.location="login.php"</script>';
					...
					$iquery="INSERT INTO `restaurant_info`( `restaurent_name`, `email`, `phone`, `address`, `location`, `logo`, `password`, `role`) 
		        		VALUES ('$fullname','$email','$phone', '$address','$area', '$file_name','$password','$role');";
		        	if ($con->query($iquery) === TRUE) {
		        		echo '<script>alert("Nuevo restaurante anadido satisfactoriamente")</script>';
		        		echo '<script>window.location="login.php"</script>';
		        	}else {
		                echo "Error: " . $iquery . "<br>" . $con->error;
		            }
    ...
    if (isset($_POST['book'])) {
       ...
       if ($bdinsert == true && $cinsert == true && $iinsert == true) {
    		echo '<script>alert("Su reserva esta hecha. Pronto recibirás un correo electrónico.")</script>';
